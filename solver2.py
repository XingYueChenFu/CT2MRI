...
def main():
    ...
    # 保存数据的位置
    output_root = '/home/featurize/output/VQ-VAE-CT'
    ...
    # 训练
    global_step = 0
    max_steps = 11520 # 样本数2304*5（相当于训练5代）
    ...
    for epoch in range(global_step, (max_steps-1) // len(trainloader) + 1):
        ...
        for i, batch in enumerate(valloader):
            ...
            if i == 0: # Log用
                realct_sample = x.cpu().numpy()
                recoct_sample = xrec.cpu().numpy()
        ...
        if eval_ssim > best_ssim:
            ...
            # 保存原图与重建图{global_step}_realCT与{}global_step}_reconCT
            np.save(os.path.join(sample_path, f"{global_step}_realCT.npy"), realct_sample)
            np.save(os.path.join(sample_path, f"{global_step}_reconCT.npy"), recoct_sample)
