...
from .reg3D import Reg
from .lossfuncs import smooothing_loss3D
from .transformer3D import Transformer_3D
...
class GANclass(nn.Module):
    def __init__(self, opt):
        ...
        self.lambda_L1 = self.L1_lambda = opt.lambda_L1
        ...
        if self.use_reg: # 不完善
            ...
            self.netR = Reg(opt.target_depth_size, opt.target_image_size, opt.target_image_size, opt.input_nc, opt.input_nc).to(self.device)
            self.optimizer_R = torch.optim.Adam(self.netR.parameters(), lr=opt.lr_max, betas=(opt.beta1, opt.beta2))
    ...
    def train_once(self):
        if self.use_reg:
            ...
            Trans = self.netR(self.real_A, self.fake_B)
            SysRegist_B = self.spatial_transformer(self.fake_B, Trans)
            fake_AB = torch.cat((self.real_A, SysRegist_B), 1)
            pred_fake = self.netD(fake_AB, isDetach=False)
            
            self.loss_G_GAN = self.criterionGAN(pred_fake, True)
            self.loss_G_L1 = self.criterionL1(SysRegist_B, self.real_B) * self.L1_lambda
            self.loss_SM = smooothing_loss3D(Trans) * self.opt.SM_lambda
            ...
            self.loss_G_loss = self.loss_G_GAN + self.loss_G_L1 + self.loss_SM + self.loss_classification
            if self.VGG_loss:
                self.loss_G_perceive = self.criterionPreLoss(SysRegist_B, self.real_B)
                self.loss_G_loss += self.loss_G_perceive
        ...
        # Fake; stop backprop to the generator by detaching self.fake_B
